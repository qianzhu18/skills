...
**默认行为**：调用 Sophnet 图像接口生成批量信息图。
**`--prompt-only`**：只输出 JSON prompt，不发起图片生成请求。
...
| `--engine` | `auto` | 引擎选择：`auto`（自动）/ `mermaid` / `sophnet` / `excalidraw` |
...
**`--engine` 参数**：
- `auto`（默认）：根据内容类型自动选择（优先级 Sophnet > Excalidraw > Mermaid）
- `sophnet`：强制只使用 Sophnet 图像接口（适合创意内容）
...
1. 读取文章内容
2. 识别配图位置（通常 3-5 个）
3. 为每个位置确定引擎（Sophnet / Excalidraw / Mermaid）
...
# Step 2: 调用脚本
SOPHNET_API_KEY=$SOPHNET_API_KEY npx -y bun ~/.codex/skills/smart-illustrator/scripts/generate-image.ts \
...
用户可直接保留 JSON prompt 供后续人工微调，不再依赖额外的 Web 出图界面。
...
article-image-01.png    # Sophnet 配图
```
