 ...
 *
 * Usage:
 *   npx -y bun ~/.codex/skills/smart-illustrator/scripts/generate-image.ts --prompt "A cute cat" --output cat.png
 *   npx -y bun ~/.codex/skills/smart-illustrator/scripts/generate-image.ts --prompt-file prompt.md --output image.png
 *
...
Examples:
  SOPHNET_API_KEY=xxx npx -y bun generate-image.ts -p "A futuristic city" -o city.png
  SOPHNET_API_KEY=xxx npx -y bun generate-image.ts -f illustration-prompt.md -o illustration.png
  SOPHNET_API_KEY=xxx npx -y bun generate-image.ts -p "A tech diagram" -r style-ref.png -o output.png
`);
  process.exit(0);
}
