...
// Sophnet multimodal API for cover analysis
...
/**
 * Analyze a cover image using Sophnet-hosted multimodal model
 */
export async function analyzeCoverImage(
  imagePath: string,
  note?: string
): Promise<CoverAnalysis | null> {
  const sophnetKey = normalizeApiKey(process.env.SOPHNET_API_KEY);
  if (!sophnetKey) {
    console.error('Error: SOPHNET_API_KEY is required for cover analysis');
    return null;
  }
  ...
  try {
    const response = await fetch(
      `${SOPHNET_API_BASE}/${ANALYSIS_MODEL}:generateContent`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${sophnetKey}`
        },
        body: JSON.stringify({
          contents: [{
            parts: [
    ...
    if (data.error) {
      console.error('Sophnet API error:', data.error.message);
      return null;
    }

    const textContent = data.candidates?.[0]?.content?.parts?.[0]?.text;
    if (!textContent) {
      console.error('No analysis result from Sophnet');
      return null;
    }
