...
1. Author 用本文件的规则设计 3 个隐喻方案（中文推导 + 英文 prompt）
2. Codex reviewer 评估：隐喻精度、prompt 质量、负向描述陷阱
3. 选择定稿 prompt
4. 直接调用 `generate-image.ts`，统一走 Sophnet 图像接口

**为什么这样做**：封面图最怕隐喻太直白、主体太满、提示词里混入无效修饰。先把 prompt 做准，再统一交给单一接口生成，稳定性会更高。
...
> 封面图默认直接使用 `generate-image.ts` 调 Sophnet 图像接口生成，不再依赖手动 Web 出图流程。
