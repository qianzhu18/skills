...
>
> 新增 Excalidraw 手绘概念图引擎。三级优先级：Sophnet 图像 → Excalidraw → Mermaid。所有图表引擎默认输出 PNG。[查看详情 →](#三引擎系统)

![三引擎架构](assets/dual-engine-architecture.png)

Claude Code 的智能文章配图 Skill，采用**三引擎系统**：根据内容类型自动选择 Sophnet 图像引擎（创意视觉）、Excalidraw（手绘图表）或 Mermaid（结构化图表）。
...
为文章配图很费时间：手动设计需要数小时，图库图片缺乏定制性，通用 AI 工具不理解文章结构。Smart Illustrator 结合智能位置检测、三引擎系统（Sophnet 图像 + Excalidraw + Mermaid）和封面学习，几分钟内生成符合上下文的插图。
...
- **三引擎系统**：根据内容类型自动选择 Sophnet 图像、Excalidraw 或 Mermaid
...
- **多后端支持**：Sophnet 图像接口生成创意视觉（2K 分辨率）、Excalidraw 生成手绘图表、Mermaid CLI 生成结构化图表，全部默认输出 PNG
...
- Sophnet API Key（必需，用于图像生成与封面分析）
...
| `--engine` | `auto` | 引擎：`auto`、`sophnet`、`excalidraw` 或 `mermaid` |
...
1. **使用 `pictures[]` 数组** - 数组结构有助于图像模型逐条执行批量生成。
...
| 输出方式 | 参数 | 说明 | 需要 API |
|----------|------|------|----------|
| 直接生成图片 | 默认 | 调用 Sophnet 图像接口生成图片 | ✅ 需要 |
| 输出 JSON Prompt | `--prompt-only` | 输出给人工微调或手动复用 | ❌ 不需要 |
...
> **💡 使用提示**：如果批量请求失败，可以先重试 1-3 次；若仍失败，再把 `pictures` 中的条目拆成单图逐条生成。
...
**限制**：
- 参考图功能通过 Sophnet 多模态接口支持
- 使用参考图时会直接随 Sophnet 请求一并发送
- 最多 3 张参考图
...
| `concept` | Excalidraw / Sophnet 图像 | 抽象概念 | 手绘 / 中心辐射 |
| `comparison` | Excalidraw / Sophnet 图像 | A vs B、对比 | 手绘 / 左右分栏 |
| `data` | Sophnet 图像 | 统计、趋势 | 信息图风格 |
| `scene` | Sophnet 图像 | 故事、场景 | 叙事插画 |
| `metaphor` | Sophnet 图像 | 类比、象征 | 创意视觉 |
| `cover` | Sophnet 图像 | 文章封面 | 16:9 深色高对比 |
...
├── LICENSE
├── scripts/
│   ├── generate-image.ts     # Sophnet 单张图片生成
│   ├── batch-generate.ts     # Sophnet 批量生成（2K、断点续传）
...
│   ├── style-light.md        # 浅色风格图像生成 Prompt（默认）
│   ├── style-dark.md         # 深色风格图像生成 Prompt
│   ├── style-minimal.md      # 极简风格图像生成 Prompt
...
使用 Sophnet 接口自动生成时，实际成本取决于你在算能科技侧的账号套餐、模型配额和请求规模。

请在运行前设置环境变量：`SOPHNET_API_KEY`。
...
- [Sophnet API](https://www.sophnet.com/) - 算能科技图像与多模态接口
