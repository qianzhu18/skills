...
>
> New Excalidraw engine for hand-drawn concept diagrams. Three-tier priority: Sophnet image engine → Excalidraw → Mermaid. All diagram engines now output PNG by default. [Details →](#tri-engine-system)

![Tri-Engine Architecture](assets/dual-engine-architecture.png)

Intelligent article illustration Skill for Claude Code with **tri-engine system**: automatically selects the Sophnet image engine (for creative visuals), Excalidraw (for hand-drawn diagrams), or Mermaid (for structured diagrams) based on content type.
...
Creating illustrations for articles is time-consuming: manual design takes hours, stock photos lack context, and generic AI tools don't understand article structure. Smart Illustrator combines intelligent position detection, tri-engine system (Sophnet image engine + Excalidraw + Mermaid), and cover learning to generate contextual illustrations in minutes.
...
- **Tri-Engine System**: Auto-selects Sophnet image generation, Excalidraw, or Mermaid based on content type
...
- **Multiple Backends**: Sophnet image API for creative visuals (2K resolution), Excalidraw for hand-drawn diagrams, Mermaid CLI for structured diagrams — all output PNG by default
...
- Sophnet API Key (required, for image generation and cover analysis)
...
| `--engine` | `auto` | Engine: `auto`, `sophnet`, `excalidraw`, or `mermaid` |
...
```bash
export SOPHNET_API_KEY=your_key
...
| `-m, --model` | Model (default: Sophnet platform image model) |
...
1. **Use `pictures[]` array** - Array structure helps the image model execute batch generation item by item.
...
|-------------|-----------|-------------|--------------|
| Generate Images | Default | Calls the Sophnet image API to generate images | ✅ Yes |
| Output JSON Prompt | `--prompt-only` | Keep the prompt for manual refinement or reuse | ❌ No |
...
> **💡 Tip**: If a batch request fails, retry 1-3 times first; if it still fails, split `pictures` into one-image requests and run them individually.
...
|----------|--------|----------|--------|
| **1** | **Sophnet Image** | Creative visuals (metaphors, scenes, infographics) | PNG (2K) |
...
**Selection logic:**
- Needs metaphor, emotion, or creative expression → Sophnet image generation
...
| `concept` | Excalidraw / Sophnet Image | Abstract concepts | Hand-drawn / Center-radial |
| `comparison` | Excalidraw / Sophnet Image | A vs B, contrasts | Hand-drawn / Left-right split |
| `data` | Sophnet Image | Statistics, trends | Infographic style |
| `scene` | Sophnet Image | Stories, scenarios | Narrative illustration |
| `metaphor` | Sophnet Image | Analogies, symbols | Creative visual |
| `cover` | Sophnet Image | Article cover | 16:9 dark tech |
...
├── LICENSE
├── scripts/
│   ├── generate-image.ts     # Sophnet single image generation
│   ├── batch-generate.ts     # Sophnet batch generation (2K, resume support)
...
│   ├── style-light.md        # Light style image-generation prompt (default)
│   ├── style-dark.md         # Dark style image-generation prompt
│   ├── style-minimal.md      # Minimal style image-generation prompt
...
### Sophnet Image Engine Parameters

| Parameter | Value | Description |
|-----------|-------|-------------|
| Model | Sophnet platform default image model | Override when needed |
...
When using the Sophnet image API, actual cost depends on your Sophnet account tier, quota, and model selection.

Set `SOPHNET_API_KEY` before running generation commands.
...
- [Sophnet API](https://www.sophnet.com/) - Sophnet image and multimodal API gateway
