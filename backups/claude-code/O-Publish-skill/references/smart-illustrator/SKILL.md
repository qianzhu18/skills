...
**默认行为**：调用 Smart Illustrator 的 Sophnet 图像接口生成批量信息图。
**`--prompt-only`**：输出 JSON prompt 并**自动复制到剪贴板**，可直接用于后续人工微调或手动复用。
...
| `--engine` | `auto` | 引擎选择：`auto`（自动）/ `mermaid` / `sophnet`（图像模式） |
| `--save-config` | - | 保存到项目配置 |
...
| 概念、隐喻、场景、对比图 | Sophnet 图像 | PNG 文件 |
| 封面图 | Sophnet 图像 | PNG 文件 |
...
1. 读取文章内容
2. 识别配图位置（通常 3-5 个）
3. 为每个位置确定引擎（Mermaid 或 Sophnet 图像）
...
# Step 2: 调用脚本
SOPHNET_API_KEY=$SOPHNET_API_KEY npx -y bun ~/.claude/skills/smart-illustrator/scripts/generate-image.ts \
...
用户可直接保留 JSON prompt 供后续人工微调或手动生成图片。
