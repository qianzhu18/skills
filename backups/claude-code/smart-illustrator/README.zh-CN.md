...
Claude Code 的智能文章配图 Skill，采用**双引擎系统**：根据内容类型自动选择 Mermaid（结构化图表）或 Sophnet 图像引擎（创意视觉）。
...
为文章配图很费时间：手动设计需要数小时，图库图片缺乏定制性，通用 AI 工具不理解文章结构。Smart Illustrator 结合智能位置检测、双引擎系统（Mermaid + Sophnet 图像）和封面学习，几分钟内生成符合上下文的插图。
...
- **双引擎系统**：根据内容类型自动选择 Mermaid 或 Sophnet 图像
...
- **多后端支持**：Mermaid CLI 生成图表，Sophnet 图像接口生成创意视觉（2K 分辨率）
...
| **Sophnet 图像** | 创意视觉（隐喻图、场景图、信息图） | 艺术、氛围、品牌化 |
...
| `concept` | Sophnet 图像 | 抽象概念 | 中心辐射 |
| `comparison` | Sophnet 图像 | A vs B、对比 | 左右分栏 |
